 ...
 */
export async function apiFind(dataType, filter) {
    let content = await fetch(`../static/json/${dataType}.json`).then(response => response.json());
    ...
    for (let key in filter) {
        ...
        for (let index in content) {
            ...
            if (entry[key].toLowerCase().includes(value))
                results.push(entry);
        }
    }
    return results;
 ...
 */
export async function apiFindExactly(dataType, filter) {
    let content = await fetch(`../static/json/${dataType}.json`).then(response => response.json());
    if (Object.keys(filter).length === 0)
        return content[0];
    let results = [];
